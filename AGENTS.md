...
- `tsconfig.dev.json` -- extends the above with `noEmit: true`; adds
  `test/**/*`, `vite.config.ts`, and `playwright.config.ts` so ESLint's
...
transform TypeScript source files on the fly. There is no browser app. The
Playwright suite is scaffolding only: `playwright.config.ts` and the `vite dev`
server exist, but `test/browser/` holds no tests.

`pnpm test` runs `fix`, `lint`, `typecheck`, `test:node`, then `test:dist`.
`test:dist` builds the package and greps the emitted `dist/` for any reference
to `@interop/logger`, failing the build if one is found -- the logging port in
`src/log.ts` must ship with no runtime dependency on that package (see
ARCHITECTURE.md's logging-port invariant).
...
Every subpath a consumer can import has a four-key entry (`types` /
`react-native` / `import` / `default`) in `package.json`'s `exports` map. The
root (`.`) re-exports every module in `src/`; `./matching` is a separate leaf
export carrying only the QueryByExample matchers, so a wallet's matching tests
load without the VP-signing and document-loader graph. A new subpath means a new
four-key entry.
...
This package is the former `request` subpath of `@interop/wallet-core`,
extracted verbatim except for the recognizer-injection and logging-port changes
ARCHITECTURE.md records. `@interop/wallet-core` imports nothing from this
package.
...
- Fields: `status` (`todo` / `in-progress` / `draft` / `done`), `priority`
  ...
  (other `WR-N` ids), a `touches:` list where the rule below applies,
...
- Completed items move **verbatim** (number, title, field block, prose, with
  ...
  keeps WR-N references resolvable. CHANGELOG.md remains the permanent record of
